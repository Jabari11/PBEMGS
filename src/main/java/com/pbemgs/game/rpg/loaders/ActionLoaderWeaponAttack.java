...
import com.pbemgs.game.rpg.combat.action.ActionWeaponAttack;
...
/**
 *  Loads ActionWeaponAttack Actions from a csv file.
 */
public class ActionLoaderWeaponAttack {
    public static Map<String, Action> loadWeaponAttacks(String csvFilePath, CombatLog combatLog) {
        Map<String, Action> actions = new HashMap<>();
        String[] expectedHeaders = {"ActionWeaponAttack", "targetType", "splashCount", "chanceToHit", "multiplier", "condition", "finisherVal"};
        ...
        try (CSVReader reader = new CSVReader(new FileReader(csvFilePath), ',', '"', 0)) {
            ...
            while ((row = reader.readNext()) != null) {
                ...
                try {
                    ...
                    String name = parseString(rowData, "ActionWeaponAttack", rowNum, true);
                    ...
                    List<Integer> finisherVal = parseFinisherValues(rowData, "finisherVal", rowNum);

                    Action action = new ActionWeaponAttack(name, targetType, splashCount, chanceToHit, multiplier, condition);
                    if (finisherVal != null) {
                        action = new ActionRogueFinisher(action, finisherVal);
                    }
