...
public class ActionLoaderArmor {
    public static Map<String, Action> loadArmorActions(String csvFilePath, CombatLog combatLog) {
        Map<String, Action> actions = new HashMap<>();
        String[] expectedHeaders = {
                "ActionArmor", "friendly", "targetType", "splashCount", "chanceToHit",
                "baseQuantity", "condition"
        };
        ...
        try (CSVReader reader = new CSVReader(new FileReader(csvFilePath), ',', '"', 0)) {
            ...
            while ((row = reader.readNext()) != null) {
                ...
                try {
                    ...
                    String name = parseString(rowData, "ActionArmor", rowNum, true);
                    ...
                    float baseQuantity = parseSignedFloat(rowData, "baseQuantity", rowNum);
                    ActionConditional condition = parseCondition(rowData.get("condition"), rowNum);

                    Action action = new ActionArmor(
                            name, friendly, targetType, splashCount, chanceToHit, baseQuantity, condition);
